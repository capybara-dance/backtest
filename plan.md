...
사람마다 중요하게 생각하는 포인트가 다르므로, 스코어링 가중치를 성향별로 달리하여 **3가지 성향의 Top 전략**을 각각 찾는 것이 최종 목표다.

| 성향 | 핵심 목표 | 특징 |
|------|-----------|------|
| 🚀 **공격형** | 수익률(CAGR) 극대화 | 높은 MDD 허용, 고위험·고수익 추구 |
| ⚖️ **중립형** | 수익·리스크 균형 | 수익성과 안정성을 고루 중시 |
| 🛡️ **안정형** | 낙폭(MDD) 최소화 | 손실 회피 최우선, 꾸준하고 방어적인 성과 |

각 세션 종료 시 성향별로 독립된 Top 포트폴리오 리스트를 도출한다.
...
[4] 종합 스코어 계산 — 성향별 3개 스코어 산출 (0~100점)
    각 지표를 전체 test_history 대비 백분위수로 정규화한 후,
    성향별 가중치를 적용하여 3개 스코어를 독립 계산한다.

    score_공격형 = 0.55*score(CAGR) + 0.10*score(-MDD) + 0.20*score(Sharpe) + 0.15*score(Calmar)
    score_중립형 = 0.35*score(CAGR) + 0.30*score(-MDD) + 0.20*score(Sharpe) + 0.15*score(Calmar)
    score_안정형 = 0.15*score(CAGR) + 0.50*score(-MDD) + 0.25*score(Sharpe) + 0.10*score(Calmar)

    record["scores"] = {"공격형": ..., "중립형": ..., "안정형": ...}
    record["score"]  = record["scores"]["중립형"]  # 하위 호환
...
1. **성향별** 상위 10개 포트폴리오 선정 (공격형 Top10 / 중립형 Top10 / 안정형 Top10)
2. 섹터/자산군 분석 → 각 성향 고스코어 포트폴리오의 공통 특성 파악
3. **`insights_YYYYMMDD_N.md`** 생성 — 성향별 Top 테이블 포함 (누적 세션 번호 포함)
...
| `metrics` | 성과 지표 |
| `score` | 종합 스코어 (0~100) — 중립형 스코어, 하위 호환용 |
| `scores` | 성향별 스코어 (`공격형`, `중립형`, `안정형` 각각 0~100) |
| `notes` | 전략 설명 및 특이사항 |
...
동일한 정규화 지표에 **성향별로 다른 가중치**를 적용하여 3개의 독립 스코어를 산출한다.

| 성향 | CAGR 가중치 | MDD 가중치 | 샤프 가중치 | 칼마 가중치 | 특징 |
|------|:-----------:|:----------:|:-----------:|:-----------:|------|
| 🚀 공격형 | **0.55** | 0.10 | 0.20 | 0.15 | 수익률 극대화, 손실 허용 |
| ⚖️ 중립형 | 0.35 | 0.30 | 0.20 | 0.15 | 수익·리스크 균형 |
| 🛡️ 안정형 | 0.15 | **0.50** | **0.25** | 0.10 | 손실 최소화, 안정 중시 |

```python
# 구현 예시 (run_agent_backtest.py)
...
for profile, w in INVESTOR_PROFILES.items():
    scores[profile] = (w["cagr"]   * score_cagr
                     + w["mdd"]    * score_mdd
                     + w["sharpe"] * score_sharpe
                     + w["calmar"] * score_calmar)
```

`record["score"]` 는 **중립형 스코어**로 설정되어 하위 호환성을 유지한다.
